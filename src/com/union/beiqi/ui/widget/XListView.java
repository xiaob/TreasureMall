 ...
 */
package com.union.beiqi.ui.widget;
...
public class XListView extends ListView implements OnScrollListener
{
	...
	private void initWithContext(Context context)
	{
		...
		// mFooterNoData = View.inflate(context, R.layout.panel_error_nodata,
		// null);
		// init header height
	 ...
	 * @param size
	 */
	public void togglePullLoadMoreByDataCount(int size)
	{
		checkDataCount(size); // Auto support for no data case.
	 ...
	 * @param size
	 */
	private void checkDataCount(int size)
	{
		if (size == 0)
		{ // 无数据
			showNoDataView();
	...
	private void showNoDataView()
	{
		...
		if (!isAddNoData)
		{
			...
			mFooterNoData.setMinimumHeight(getHeight()); // Make sure no data
															// view is full
															// content
			isAddNoData = true;
			setClickable(false);
	...
	private void hideNoDataView()
	{
		if (isAddNoData)
		{
			removeFooterView(mFooterNoData);
			isAddNoData = false;
			setClickable(true);
		}
		if (!mIsFooterReady)
